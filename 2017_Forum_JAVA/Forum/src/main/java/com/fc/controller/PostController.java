...
public class PostController {
    ...
    public String toPost(int pid,Model model,HttpSession session){
        ...
        //向模型中添加数据
        String notice = topicService.othersT1(4);
    ...
    @RequestMapping("/topicpost.do")
    public String toPosts(int ttid,Model model){
        List<Post> postList = postService.getPostListByTid(ttid);
        List<User> userList = userService.listUserByTime();
        List<User> hotUserList = userService.listUserByHot();
        Topic TopicName = topicService.getTopicNameById(ttid);
